...
#     启动时自动 seed 8 SKU + 开账 + 3 订单.
#   - 光天后端 (dev_guangtian_backend.py) 暂未进 main (PR #122 review 中), 只在
#     `guangtian-backend` worktree → 脚本会 probe sibling worktree 找它.
#   - 前端用 `guangtian-frontend` worktree (同时带 jintai + guangtian 两 tab).
#   - Logs: /tmp/guangtian-demo-{backend,vite}.log; PIDs: /tmp/guangtian-demo-{backend,vite}.pid
#   - Override: $DEMO_BACKEND_DIR / $DEMO_WEB_DIR / $GUANGTIAN_PYTHON.

set -u

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"

# --- backend probe: 找含 dev_guangtian_backend.py 的 services/platform-api ---
_has_guangtian_backend() { [[ -f "$1/dev_guangtian_backend.py" ]]; }
BACKEND_DIR=""
for cand in \
  "${DEMO_BACKEND_DIR:-}" \
  "$ROOT/services/platform-api" \
  "$ROOT/../guangtian-backend/services/platform-api" \
  ; do
  [[ -z "$cand" ]] && continue
  if _has_guangtian_backend "$cand"; then BACKEND_DIR=$(cd "$cand" && pwd); break; fi
done
if [[ -z "$BACKEND_DIR" ]]; then
  echo "ERR: cannot locate dev_guangtian_backend.py."
  echo "     光天后端尚未进 main, 需要 guangtian-backend worktree, 或 set \$DEMO_BACKEND_DIR."
  echo "     Tried:"
  echo "       \$DEMO_BACKEND_DIR (${DEMO_BACKEND_DIR:-unset})"
  echo "       $ROOT/services/platform-api"
  echo "       $ROOT/../guangtian-backend/services/platform-api"
  exit 1
fi

# --- frontend probe: 找含 GuangtianDemoPage.tsx 的 apps/win-web ---
# 用屏幕文件存在与否判断, 而非 apps/win-web 目录存在 (旧 win-web 不含 ?tab=guangtian
# 路由会白屏). guangtian-frontend 同时带 jintai + guangtian 两 tab.
...
for candidate in \
  ...
  if _has_guangtian_screens "$candidate"; then WEB_DIR=$(cd "$candidate" && pwd); break; fi
done
if [[ -z "$WEB_DIR" ]]; then
  ...
  echo "     Tried:"
  echo "       \$DEMO_WEB_DIR (${DEMO_WEB_DIR:-unset})"
  echo "       $ROOT/apps/win-web"
  echo "       $ROOT/../guangtian-frontend/apps/win-web"
  echo "       $ROOT/../guangtian-frontend-v2/apps/win-web"
  exit 1
fi
