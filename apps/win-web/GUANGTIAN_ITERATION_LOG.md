...
| Tab | 核心需求 | 第一性原理状态 |
|---|---|---|
| 1. 工作台 | 全景索引（4 需求入口） | KPI 需调整 → 第 4 张"异常"改"今日出入库" |
| 2. SKU 档案 | **知道有多少** | 列过多（9 列含材质冗余）→ 减到 7 列 |
| 3. 入库登记 | **每笔可查** | 表单已折叠核心 5 字段（G9） · 保留 |
| 4. 出库登记 | **每笔可查** | 库存校验红警告 + 关联订单 · 保留 |
| 5. 库存流水 | **每笔可查 / 可追溯** | 9 列含 AI 置信度 + 已确认（G11） · 保留 |
| 6. 缺货预警 | **缺货提前知道** | 4 级紧急级 + 进度条（G11） · 保留 |
| 7. AI 补产 | **缺货提前知道** | 3 SKU + AI 1 句理由（G9） · 保留 |
| 8. 问问 AI | **老板直接问** | 5 spec 预设 + 结构化答案（G11） · 保留 |
| 9. 库存日报 | **老板直接问** | 5 块（G9） · 保留 |

### 本轮实际改动

**1. Hero 副标重写** — 直接亮 4 核心承诺：
"知道有多少 · 每笔可查 · 缺货提前知道 · 老板直接问 — AI 替您管 1,000+ SKU。"

**2. Dashboard KPI #4 换牌**：异常记录 → **今日出入库** `18/23`（联动 todayInboundCount / todayOutboundCount）。异常归属流水 tab。

**3. SKU 表 9 列 → 7 列**：
- 删"材质"列（category 字段已含品类）
- "最近入库"+"最近出库" 合并成单列"最近动销"，堆叠 `入X / 出Y` 显示
- 删除 `MATERIAL_MAP` 死代码

### 数学验证
| 改动 | Before | After |
|---|---:|---:|
| Hero 副标用词 | 抽象（管清楚 / 提前 3 天） | 4 核心需求逐条点名 |
| Dashboard KPI 出入库相关数 | 2/4 (低库 + 缺货) | **3/4** (+今日出入库) |
| SKU 表列数 | 9 | **7** (-22%) |
