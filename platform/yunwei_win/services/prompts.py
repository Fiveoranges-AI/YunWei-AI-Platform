...
``yunwei_win`` package lands in site-packages, but ``prompts/`` is copied
...
When the RQ worker runs via the ``yunwei-win-ingest-worker`` console script,
...
def find_prompt(filename: str) -> Path:
    ...
    1. ``$PROMPTS_DIR`` env (operator override; handy in tests + future
       container layouts).
    2. ``yunwei_win/../prompts/<filename>`` — works when the package
       lives in a source tree where ``prompts/`` sits next to
       ``yunwei_win/`` (local dev and the ``/app`` layout).
    3. ``/app/prompts/<filename>`` — the Railway container layout, used
       when ``yunwei_win`` is imported from site-packages.
    ...
    # parents[1] = yunwei_win/, then ../prompts is the source-tree layout.
