...
**延后清单** (P2/P3 backlog,需新增 scope):
| 项 | 建议下一步 |
|---|---|
| payment_out / cash_movements 事件表 | round 3 (单独 PR,~ 1 天) |
| 折旧细分到制造/销售/管理费用 | 加 FA.usage_category 字段 + PNL 分流 (~ 0.5 天) |
| BOM auto-consume → 批量出库 | 单独 PR (~ 0.5 天) |
| LLM 真调 auto-draft (现 rule-based) | prompts.py 加 schema-aware prompt (~ 1 天) |
| 接入 Kingdee K3 抽取已有三表 | 银湖项目分支正在做,等接口确定 |
| WAC → FIFO (lot tracking) | 大工程,等真实需求 |
| ActionTargetType 扩展 procurement 值 | PG enum migration (需谨慎,~ 0.5 天) |
| chart_of_accounts 扩展锦泰真实科目 | 等业务方拿真表 |
